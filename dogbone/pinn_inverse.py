...
nu_init = 0.28  # Initial guess for Poisson's ratio
...
def boundary_right(x, on_boundary):
    return on_boundary and dde.utils.isclose(x[0], x_max_ROI)
...
def HardBC(x, f, x_max=x_max_ROI):
...
def solution_fn(x):
    if net_type == "spinn":
        ...
        x_mesh = [x_.ravel() for x_ in jnp.meshgrid(x[0].squeeze(), x[1].squeeze(), indexing="ij")]
        x = stack(x_mesh, axis=-1)
        sol = np.array([interp((x[:,0]+offs_x, x[:,1]+offs_y)) for interp in interpolators]).T
    return sol
...
n_integral = 200
x_integral = np.linspace(0, x_max_ROI, n_integral).reshape(-1, 1)
y_integral = np.linspace(0, y_max_FEM, n_integral).reshape(-1, 1)
...
Integral_BC = dde.PointSetOperatorBC(integral_points, pstress * x_max_ROI, integral_stress) # stress ntegral of m*x + b
...
DIC
"""
X_DIC_input = [np.linspace(0, x_max_ROI , n_DIC).reshape(-1, 1),
               np.linspace(0, y_max_ROI, n_DIC).reshape(-1, 1)]
...
if net_type == "spinn":
    ...
    X_plot = [np.linspace(0, x_max_ROI, 100).reshape(-1,1),
              np.linspace(0, y_max_ROI, 100).reshape(-1,1)]
