...
parser.add_argument('--n_DIC', type=int, default=64, help='Number of DIC') # n_DIC**2 points
...
parser.add_argument('--loss_weights', nargs='+', type=float, default=[1,1,1,1,1,1,1e8,1e8], help='Loss weights (more on DIC points)') # Ux, Uy, Sxx, Syy, Sxy, Integral, DIC ?
...
E_init = 180e3 # Initial guess for Young's modulus
...
def boundary_left(x, on_boundary):
    return on_boundary and dde.utils.isclose(x[0], offs_x)


def boundary_right(x, on_boundary):
    return on_boundary and dde.utils.isclose(x[0], offs_x + x_max_ROI)


def boundary_bottom(x, on_boundary):
    return on_boundary and dde.utils.isclose(x[1], offs_y)
...
def HardBC(x, f, x_max=offs_x + x_max_ROI):
    ...
    Syy = f[:, 3] #* (y_max_ROI + offs_y - x[:, 1]) * (offs_y - x[:, 1])
...
data = np.loadtxt(os.path.join(dir_path, r"./fem_solution_dogbone.dat"))

"""
test filtering data
"""
# data = data[(data[:, 0] >= offs_x) & (data[:, 0] <= offs_x + b) & 
#                      (data[:, 1] >= offs_y) & (data[:, 1] <= offs_y + L_u)]
...
x_grid = np.linspace(0, x_max_FEM, n_mesh_x)
y_grid = np.linspace(0, y_max_FEM, n_mesh_y)
# x_grid = np.linspace(0, x_max_ROI, n_mesh_x)
# y_grid = np.linspace(0, y_max_ROI, n_mesh_y)
...
def solution_fn(x):
    if net_type == "spinn":
        ...
        x_mesh = [x_.ravel() for x_ in jnp.meshgrid(x[0].squeeze(), x[1].squeeze(), indexing="ij")]
        x = stack(x_mesh, axis=-1)
        sol = np.array([interp((x[:,0], x[:,1])) for interp in interpolators]).T
    return sol
...
Integral_BC = dde.PointSetOperatorBC(integral_points, pstress * x_max_ROI, integral_stress)
...
if net_type == "spinn":
    ...
    X_plot = [np.linspace(offs_x, offs_x + x_max_ROI, 100).reshape(-1,1),
              np.linspace(offs_y, offs_y + y_max_ROI, 100).reshape(-1,1)]
    ...
    num_params = get_num_params(net, input_shape=layers[0])
    X_mesh = np.meshgrid(
        np.linspace(0, 1, x_max_ROI, dtype=np.float32),
        np.linspace(0, 1, y_max_ROI, dtype=np.float32),
        indexing="ij",
    )
