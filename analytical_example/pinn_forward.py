"""
PINN and SPINN implementation of the linear elasticity 2D toy example introduced in paper https://doi.org/10.1016/j.cma.2021.113741.
"""
...
net_type = ["spinn", "pfnn"][0] # SPINN only implemented with JAX backend and HardBC

n_iter = int(1e6) # Large number, will be stopped by the Timer callback
log_every = 100
...
if net_type == "spinn":
    ...
    num_point = 64**2
    total_points = num_point 
    num_params = get_num_params(net, input_shape=layers[0])
    X_plot = [np.linspace(0, 1, 100).reshape(-1, 1)] * 2 #SPINN takes list of 1D arrays (X_coords, Y_coords) as input
...
results_path = os.path.join(dir_path, "results_forward")
...
def log_config(fname):
    ...
    info = {**system_info, **execution_info}
